...
For more information, visit [ggplot2 link](https://ggplot2.tidyverse.org/)
...
Below, we use `geom_dotplot()` to plot dot plot of math scores.
...
The function `scale_y_continuous()` is also added to turn off the y-axis by setting to NULL.
...
We modify the previous plot using 3 arguments:-
...
The code below changes the interior colour of the histogram (i.e. *fill*) by using sub-group of `aes()`.
...
The code below add mean values by using `stat_summary().`
...
Using `coord_flip()` we can flit the horizontal bar chart to a vertical one.
...
The code below plats a horizontal bar chart using `theme_gray()`.
...
We can further customize the charts by adding additional arguments using `theme().`
...
# 1.12 Plotting Practise

Some samples of modifications made
