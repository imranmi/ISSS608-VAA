...
These means scores can be corroborated at the following [link (pages 310-315)](https://read.oecd-ilibrary.org/education/pisa-2022-results-volume-i_bc9c7189-en#page1).
...
print(Parents_edu_read)
print(Parents_edu_scie)
```
...
We examine the new tables created. In these new tables we are able to extract the percentage breakdown of students scores for our next visualization.

```{r}
print(Math_Breakdown)
print(Read_Breakdown)
print(Scie_Breakdown)
```

We will plot bar charts for the breakdown of scores for each subject.
