...
I have chosen this source over [GADM](https://gadm.org/data.html) and [GeoBoundaries](https://www.geoboundaries.org/) due to its updated administrative region information and map levels.
...
\
Some names of administrative areas were found to have changed; either disaggregated into new administrative areas or previously active but now defunct. Some administrative areas were also aggregated into higher administrative areas.
...
I will first loading in the shape files at the admin 1 (region/state) and admin 2 (districts) levels. Most of our plots will be utilizing admin 2 levels.
...
For the module on **Cluster & Outlier Analysis (LISA)**, I will first be creating both Choropleth and Proportional Symbol Maps, as part of the initial Interactive Exploratory Analysis.
...
![](images/clipboard-3061140252.png)
...
-   The Choropleth map will be able to communicate the "intensity" of the events through use of color gradient. Users will be able to select the specific year, event type, count type (incidents or fatalities) and the data classification method.
...
In the next part of our UI, we will be exploring Spatial statistics.
...
For the sake of readibility, it may also be a good idea to add a data table of the values, for users to make sense of both maps.
...
![](images/clipboard-3240170765.png){width="717"}
...
As mentioned, for the next tab of our Cluster and Outlier Analysis module, I will be creating Moran Scatter plots and the LISA cluster map.
...
![](images/clipboard-2958084756.png)
...
## 4.11.1 Shiny Code considerations
...
This is an alternative spatial statistics for detecting spatial anomalies. It looks at neighbours within a defined proximity to identify where either high or low values clusters spatially.
...
The code below plots the Gi values derived using fixed distance weight matrix, alongside a standard choropleth map of incidents of Battles in 2023.
...
The code below plots the Gi values derived using adaptive distance weight matrix, alongside a standard choropleth map of incidents of Battles in 2023.
...
Both the choropleth map and data table will be used to populate our app's Hot & Cold Spot Analysis module.

The motivation here is to provide users with a Visualisation of statiscally significant areas of event types in order to help users understand how hot and cold zones may have developed and/or changed over time.

The below is a visual of the prototype.

![](images/clipboard-3293581756.png)

[**Functionality and interactivity**]{.underline}

-   Users will only need to select the year and the event type once. Both the map and the data table will be updated upon user selection.

-   For the choropleth map, users will be able to select the specific year, event type, the data classification method and the number of clusters for the adaptive weight matrix.
...
Additionally, I will first implement adaptive distance weights, instead of having both fixed and adaptive. This is due the the additional complexity of the shiny code required to accomodate the selection of either adaptive or distance weights within the same reactive statement.

## 4.13.1 Shiny code considerations
